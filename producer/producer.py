...
if __name__ == '__main__':
    ...
    credentials = pika.PlainCredentials(os.environ.get('RABBITMQ_USERNAME', 'user'), os.environ.get('RABBITMQ_PASSWORD', 'user'))
    ...
    LOG.info(f'going to run {int(args.repeat)} times')
    for i in range(0, int(args.repeat)):
        try:
            publish_successful = channel.basic_publish(exchange='', routing_key=q_name, body=args.message)
            if publish_successful:
                LOG.info('Message has been successfully published')
            else:
                LOG.warning('Message publishing failed')
        except Exception as e:
            LOG.error(f"An error occurred while publishing the message: {str(e)}")
